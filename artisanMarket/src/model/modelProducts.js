...
const modelProducts = {
    getProducts: function() {
        return JSON.parse(fs.readFileSync(path.resolve(__dirname, '../data/products.json'), { encoding: "utf-8" }));
    },
    isExist: function(id) {
    ...
    createProduct: function(newProduct) {
        this.getProducts().push(newProduct);
            fs.writeFileSync(path.resolve(__dirname, '../data/products.json'), JSON.stringify(DB, null, 4), { encoding: "utf-8" });
    ...
    updateProduct: function(productId, productUpdated) {
        ...
        if (indiceBuscado < 0) {
            ...
            fs.writeFileSync(path.resolve(__dirname, '../data/products.json'), JSON.stringify(newDB, null, 4), { encoding: "utf-8" });
